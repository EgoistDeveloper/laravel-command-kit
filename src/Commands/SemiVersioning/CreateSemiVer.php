...
namespace LaravelCommandKit\Commands\SemiVersioning;
...
class CreateSemiVer extends GeneratorCommand
{
    /**
     ...
     * @var string
     */
    protected $signature = 'semiver {name} {--table=} {--controller=true} {--use-base-folder=false}';
    ...
    protected $defaultTypes = [
        ...
        [
            ...
            'suffix' => '',
            'path' => 'App\Models',
            'stub' => 'app\Console\CustomCommands\MicroApp\Stubs\model.stub',
        ...
        [
            ...
            'stub' => 'app\Console\CustomCommands\MicroApp\Stubs\repository.stub',
            'use_base_folder' => false
        ], // request
        [
            'type' => 'request',
            ...
            'stub' => 'app\Console\CustomCommands\MicroApp\Stubs\request.stub',
        ...
        [
            ...
            'stub' => 'app\Console\CustomCommands\MicroApp\Stubs\controller.stub',
            'use_base_folder' => true
        ]
    ];

    protected $askableTypes = [
        'migration' => [
            'type' => 'migration',
            'suffix' => '',
            'path' => 'database\migrations',
            'stub' => 'app\Console\Stubs\migration.stub'
        ]
    ];
    ...
    private function modelScript($type, $file)
    {
        ...
        if ($argTable) {
            ...
            if ($result) {
                ...
                $path = $this->askableTypes['migration']['path'];
                //$className = $this->pluralize($file['class_name']);
                $className = ucfirst($file['class_name']);
                $date = date('Y_m_d_His');

                $migrationFile = base_path("{$path}\\*_{$argTable}.php");
                ...
                if ($askMigration && ($askMigration == 'yes' || $askMigration == 'y')) {
                    $type = $this->askableTypes['migration'];

                    $file = [
                        'path' => $path,
                        'class_name' => $className,
                        'file_path' => base_path("{$path}\\{$date}_{$argTable}.php"),
                        'stub_path' => base_path($type['stub'])
                    ];
    ...
    private function migrationIsExists($tableName){
        $path = base_path($this->askableTypes['migration']['path']);
        ...
        return preg_grep("/^([0-9_]+)({$tableName}).php/", $fileNames) ? true : false;
    }
